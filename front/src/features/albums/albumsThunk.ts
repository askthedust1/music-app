...
export const fetchAlbums = createAsyncThunk('albums/fetchAll', async (id: string) => {
  const albumResponse = await axiosApi.get<IAlbumType | null>(`/albums?artist=${id}`);
  return albumResponse.data;
});

export const fetchAdminAlbums = createAsyncThunk('albums/fetchAdmin', async () => {
  const albumResponse = await axiosApi.get<IAlbumAdmin[]>(`/albums/admin`);
  return albumResponse.data;
});

export const delAlbums = createAsyncThunk('albums/delAlbums', async (_id: string) => {
  await axiosApi.delete(`/albums/${_id}`);
});

export const patchAlbums = createAsyncThunk('albums/patchAlbums', async (_id: string) => {
